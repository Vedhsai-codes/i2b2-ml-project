-- ============================================================================
-- cohort_ckd_temporal.sql — incident chronic-kidney-disease cohort with a
-- BLACKOUT-window feature cutoff, for the leakage / lead-time ablation.
--
-- One row per patient.
--   label = 1  incident CKD: patient's FIRST CKD admission (ICD-10 N18 / ICD-9 585),
--              AND has >=1 prior admission (so pre-onset history exists). Because
--              onset = first CKD code, no CKD exists before the anchor by
--              construction, so the self-comorbidity cm_ckd is structurally absent
--              pre-onset — and it is DROPPED from the feature set entirely (below)
--              to avoid the concurrent (blackout 0) admission leaking the label.
--   label = 0  never-CKD patient with >=2 admissions.
--   anchor_time = CKD onset admittime (cases) / last admittime (controls).
...
--
-- CAVEAT (chronic disease): CKD is chronic and "first coded" is not biological
-- onset; read label=1 as *newly documented* CKD. Serum creatinine (kept as a
-- feature) is the diagnostic basis, so the concurrent design is expected to be
-- strong; the ablation shows how much is recency vs. genuine lead time.
--
-- Vary BLACKOUT_PLACEHOLDER in {0, 30, 90, 180}:
--   0   -> features from the index/anchor (CKD) admission itself = CONCURRENT (leaky)
--   30/90/180 -> features from a hospital visit >= N days before CKD onset.
...
-- (self-comorbidity cm_ckd intentionally omitted — it is the label)
comorbid AS (
  SELECT f.subject_id,
