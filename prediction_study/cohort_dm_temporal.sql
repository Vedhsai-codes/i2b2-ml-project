-- ============================================================================
-- cohort_dm_temporal.sql — incident diabetes cohort with a BLACKOUT-window
-- feature cutoff, for the leakage / lead-time ablation.
--
-- One row per patient.
--   label = 1  incident diabetes: patient's FIRST diabetes admission
--              (ICD-10 E08/E09/E10/E11/E13 / ICD-9 250 — "any diabetes", matching
--              the harmonized MIMIC<->eICU identification label), AND has >=1 prior
--              admission. Onset = first diabetes code, so the self-comorbidity cm_dm
--              is structurally absent pre-onset and is DROPPED from the feature set
--              (below) to keep the concurrent (blackout 0) admission from leaking it.
--   label = 0  never-diabetes patient with >=2 admissions.
--   anchor_time = diabetes onset admittime (cases) / last admittime (controls).
...
--
-- CAVEAT (chronic disease): diabetes is chronic and "first coded" is not biological
-- onset; read label=1 as *newly documented* diabetes. Serum glucose / HbA1c (kept as
-- features) are the diagnostic basis, so the concurrent design is expected to be
-- strong; the ablation shows how much is recency vs. genuine lead time.
--
-- Vary BLACKOUT_PLACEHOLDER in {0, 30, 90, 180}:
--   0   -> features from the index/anchor (diabetes) admission itself = CONCURRENT (leaky)
--   30/90/180 -> features from a hospital visit >= N days before diabetes onset.
...
dx AS (
  ...
  WHERE (icd_version = 10 AND (icd_code LIKE 'E08%' OR icd_code LIKE 'E09%' OR icd_code LIKE 'E10%' OR icd_code LIKE 'E11%' OR icd_code LIKE 'E13%'))
...
onset AS (
  ...
  FROM adm a JOIN dx h USING (subject_id, hadm_id)
...
controls AS (
  ...
  WHERE a.subject_id NOT IN (SELECT subject_id FROM onset)
    AND na.n >= 2
  GROUP BY a.subject_id
),
...
-- (self-comorbidity cm_dm intentionally omitted — it is the label)
comorbid AS (
  SELECT f.subject_id,
