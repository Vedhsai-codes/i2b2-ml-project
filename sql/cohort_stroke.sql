...
--
-- Cases: earliest admission carrying a label ICD code (Ischemic stroke model trained on MIMIC (silver standard ICD-10 I63/I64)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'I63%' OR d.icd_code LIKE 'I64%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '433%' OR d.icd_code LIKE '434%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'I60%' OR d.icd_code LIKE 'I61%' OR d.icd_code LIKE 'I62%' OR d.icd_code LIKE 'I63%' OR d.icd_code LIKE 'I64%' OR d.icd_code LIKE 'I65%' OR d.icd_code LIKE 'I66%' OR d.icd_code LIKE 'I67%' OR d.icd_code LIKE 'I68%' OR d.icd_code LIKE 'I69%' OR d.icd_code LIKE 'G45%' OR d.icd_code LIKE 'G46%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '430%' OR d.icd_code LIKE '431%' OR d.icd_code LIKE '432%' OR d.icd_code LIKE '433%' OR d.icd_code LIKE '434%' OR d.icd_code LIKE '435%' OR d.icd_code LIKE '436%' OR d.icd_code LIKE '437%' OR d.icd_code LIKE '438%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
...
case_idx AS (
  ...
  FROM (
    ...
    WHERE ((d.icd_version = 10 AND (d.icd_code LIKE 'I63%' OR d.icd_code LIKE 'I64%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '433%' OR d.icd_code LIKE '434%')))
  )
  WHERE rn = 1
),
