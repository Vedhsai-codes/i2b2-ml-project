...
--
-- Cases: earliest admission carrying a label ICD code (Obstructive sleep apnea on MIMIC (silver standard ICD-10 G47.33)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'G4733%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '32723%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'G473%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '3272%' OR d.icd_code LIKE '78057%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
...
case_idx AS (
  ...
  FROM (
    ...
    WHERE ((d.icd_version = 10 AND (d.icd_code LIKE 'G4733%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '32723%')))
  )
  WHERE rn = 1
),
