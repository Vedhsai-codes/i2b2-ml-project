...
--
-- Cases: earliest admission carrying a label ICD code (@@LABEL_HUMAN@@).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN @@LABEL_PREDICATE@@ THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN @@NEG_EXCLUDE_PREDICATE@@ THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
