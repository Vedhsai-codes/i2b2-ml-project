...
--
-- Cases: earliest admission carrying a label ICD code (Asthma on MIMIC (silver standard ICD-10 J45)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'J45%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '493%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'J45%' OR d.icd_code LIKE 'J44%' OR d.icd_code LIKE 'J46%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '493%' OR d.icd_code LIKE '496%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
...
case_idx AS (
  ...
  FROM (
    ...
    WHERE ((d.icd_version = 10 AND (d.icd_code LIKE 'J45%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '493%')))
  )
  WHERE rn = 1
),
