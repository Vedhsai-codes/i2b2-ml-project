...
--
-- Cases: earliest admission carrying a label ICD code (Type 2 diabetes on MIMIC (silver standard ICD-10 E11)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'E11%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'E08%' OR d.icd_code LIKE 'E09%' OR d.icd_code LIKE 'E10%' OR d.icd_code LIKE 'E11%' OR d.icd_code LIKE 'E13%' OR d.icd_code LIKE 'R73%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '250%' OR d.icd_code LIKE '7902%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
