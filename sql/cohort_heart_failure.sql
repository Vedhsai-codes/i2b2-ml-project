...
--
-- Cases: earliest admission carrying a label ICD code (Heart-failure model trained on MIMIC (silver standard ICD-10 I50)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'I50%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '428%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'I50%' OR d.icd_code LIKE 'I110%' OR d.icd_code LIKE 'I130%' OR d.icd_code LIKE 'I132%' OR d.icd_code LIKE 'I255%' OR d.icd_code LIKE 'I420%' OR d.icd_code LIKE 'I429%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '428%' OR d.icd_code LIKE '4254%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
...
case_idx AS (
  ...
  FROM (
    ...
    WHERE ((d.icd_version = 10 AND (d.icd_code LIKE 'I50%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '428%')))
  )
  WHERE rn = 1
),
