...
--
-- Cases: earliest admission carrying a label ICD code (Chronic kidney disease on MIMIC (silver standard ICD-10 N18)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'N18%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '585%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'N18%' OR d.icd_code LIKE 'N19%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '585%' OR d.icd_code LIKE '586%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
...
case_idx AS (
  ...
  FROM (
    ...
    WHERE ((d.icd_version = 10 AND (d.icd_code LIKE 'N18%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '585%')))
  )
  WHERE rn = 1
),
