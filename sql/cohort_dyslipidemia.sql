...
--
-- Cases: earliest admission carrying a label ICD code (Dyslipidemia / hyperlipidemia on MIMIC (silver standard ICD-10 E78)).
...
flags AS (
  SELECT
    subject_id,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'E78%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '272%'))) THEN 1 ELSE 0 END)        AS is_case,
    MAX(CASE WHEN ((d.icd_version = 10 AND (d.icd_code LIKE 'E78%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '272%'))) THEN 1 ELSE 0 END)  AS is_excluded
  FROM dx d
  GROUP BY subject_id
),
...
case_idx AS (
  ...
  FROM (
    ...
    WHERE ((d.icd_version = 10 AND (d.icd_code LIKE 'E78%')) OR (d.icd_version = 9 AND (d.icd_code LIKE '272%')))
  )
  WHERE rn = 1
),
